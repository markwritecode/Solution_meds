...
<body class="login">
  <form method="POST" id="login_form" autocomplete="off" action="../main/main_screen.php?auth=login&site=<?php echo attr($_SESSION['site_id']); ?>" target="_top" name="login_form">
      <div class="<?php echo $loginrow; ?> pl-3">
          <div class="<?php echo $formarea; ?>">
            ...
            <div class="form-group oe-pull-away">
                <button type="submit" class="btn btn-login btn-lg" onClick="transmit_form(this)" id="login-btn"><i class="fa fa-sign-in-alt"></i>&nbsp;&nbsp;<?php echo xlt('Login');?></button>
            </div>
          </div>
          <div class="<?php echo $logoarea; ?>">
            ...
            <?php } else { ?>
              <div class="mx-auto m-4 w-75">
                  ...
                  <img src="/public/images/med-logo.jpg" alt="" style="width:200px; height:100px; margin-left:-30px;">
              </div>
            <?php } ?>
            <div class="text-center login-title-label">
                ...
                <?php } ?>
            <!--<p class="text-center lead font-weight-normal login-bg-text-color"><?php echo xlt('The Front Desk'); ?></p>-->
            <!--<p class="text-center small"><a href="#" class="login-bg-text-color" target="main"><?php echo xlt('Password: pass'); ?></a></p>-->
            <div class="form-group">
                <label for="login_user">Login User Level</label>
                <select class="form-control" id="login_user">
...
<script>
    ...
    $('#login_user').change((e)=>{
        if($( "#login_user option:selected" ).text() =="Laboratory" && login_user[1].username == 'lab' ){
            $('#authUser').val(login_user[1].username);
            $('#clearPass').val(login_user[1].password);
            document.getElementById('login-btn').click();
            return 0;
        }
        
        if($( "#login_user option:selected" ).text() =="Accountant" && login_user[3].username == 'account' ){
            $('#authUser').val(login_user[3].username);
            $('#clearPass').val(login_user[3].password);
            document.getElementById('login-btn').click();
            return 0;
        }
        
        if($( "#login_user option:selected" ).text() =="Medical Doctor" && login_user[2].username == 'doctor' ){
            $('#authUser').val(login_user[2].username);
            $('#clearPass').val(login_user[2].password);
            document.getElementById('login-btn').click();
            return 0;
        }
        
        if($( "#login_user option:selected" ).text() =="Medical Nurse" && login_user[4].username == 'Nurse' ){
            $('#authUser').val(login_user[4].username);
            $('#clearPass').val(login_user[4].password);
            document.getElementById('login-btn').click();
            return 0;
        }
        
        if($( "#login_user option:selected" ).text() =="Front Desk" && login_user[0].username == 'frontdesk' ){
            $('#authUser').val(login_user[0].username);
            $('#clearPass').val(login_user[0].password);
            document.getElementById('login-btn').click();
            return 0;
        }
        
        if($( "#login_user option:selected" ).text() =="Pharmacy" && login_user[5].username == 'pharmacy' ){
            $('#authUser').val(login_user[5].username);
            $('#clearPass').val(login_user[5].password);
            document.getElementById('login-btn').click();
            return 0;
        }
    })
