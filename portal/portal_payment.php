                    ...
                    <?php } else { ?>
                        ...
                        <form method="post" name="payment-form" id="payment-form">
                            <fieldset>
                                ...
                                <div class="form-group">
                                    <label label-default="label-default"><?php echo xlt('Email Address'); ?></label>
                                    <div class="controls">
                                        <input name="Email" id="email-address" type="text" class="form-control"  title="<?php echo xla('Fill your email address'); ?>" value="" />
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <h4 style="display: inline-block;"><?php echo xlt('Payment Amount'); ?>:&nbsp;
                                        <strong><span id="payTotal"></span></strong>
                                    </h4>
                                </div>
                                <input type='hidden' name='mode' id='mode' value=''/>
                                <input type='hidden' name='cc_type' id='cc_type' value=''/>
    ...
    <?php if ($GLOBALS['payment_gateway'] == 'Stripe') { // Begin Include Stripe ?>
        <script>
            ...
            function payWithPaystack(e) {
                  //document.getElementById('paymentForm').preventDefault();
                  function validateEmail(email){
                        var re = /\S+@\S+\.\S+/;
                        return re.test(email);
                    }

                  var hl = validateEmail( $('#email-address').val().trim() );
                  if(!hl){
                     return 0; 
                  }
                  let handler = PaystackPop.setup({
                    key: 'pk_test_ae5da0e82de8fef9dead104cef43a3502552c2fd', // Replace with your public key
                    email: document.getElementById("email-address").value,
                    amount: parseFloat(document.getElementById("payTotal").innerHTML) * 100,
                    ...
                    callback: function(response){
                        ...
                        //alert(message);
                        handlePaystack(response);
                    }
                  });
                  handler.openIframe();
                }
                // Submit form reference number 
                function handlePaystack(paystackData){
                    
                    var paymentData = new FormData();
                    paymentData.append('mode', 'StripePayStack');
                    let inv_values = JSON.stringify(getFormObj('invoiceForm'));
                    paymentData.append('transaction_ref', paystackData.reference);
                    paymentData.append('transaction_status', paystackData.status);
                    paymentData.append('form_pid', $('#pid').val().trim());
                    paymentData.append('invValues', inv_values);
                    paymentData.append('paystackData', JSON.stringify(paystackData));
                    ...
                    $.post('/portal/lib/paylib.php', paymentData)
